...
class RationaleClusterGenerator:
  ...
  def _generate_cluster_titles(
    ...
    for attempt in range(max_retries):
      ...
      if not output_parsed:
        _logger.warning(f'Invalid output with missing <groups> tags (attempt {attempt + 1}/{max_retries})')
        # ✅ 수정: 재시도 대신 계속 진행
        if attempt < max_retries - 1:
          continue
        else:
          # ✅ 추가: 기본 클러스터 타이틀 생성
          cluster_titles = [f'Cluster {i+1}' for i in range(num_clusters)]
          _logger.warning(f'Using default cluster titles: {cluster_titles}')
      else:
        cluster_titles = [
        ...
        # ✅ 추가: 부족한 클러스터 타이틀 채우기
        if len(cluster_titles) < num_clusters:
          _logger.warning(f'Generated only {len(cluster_titles)} clusters, filling remaining with defaults')
          for i in range(len(cluster_titles), num_clusters):
            cluster_titles.append(f'Cluster {i+1}')
        
        break

    _logger.info(f'Done generating cluster titles: {cluster_titles}')
  ...
  def _embed_cluster_titles(
    ...
    if not cluster_titles:
      _logger.warning('No cluster titles to embed, returning empty list')
      return []
  ...
  def _compute_similarities_to_clusters(
    ...
    """
    # ✅ 추가: 빈 배열 체크
    if not embeddings_for_cluster_titles:
      _logger.warning('No cluster embeddings, returning zero similarities')
      return {
          rationale: []
          for rationale in embeddings_for_rationales.keys()
      }
    
    # ✅ 추가: 임베딩 차원 체크
    if not embeddings_for_rationales:
      _logger.warning('No rationale embeddings, returning empty dict')
      return {}
  ...
  def _store_similarities_to_rationale_bullets(
    ...
    for rationale_bullets_for_example in rationale_bullets_for_examples:
      ...
      for rationale in rationale_bullets_for_example:
        # ✅ 추가: 키가 없는 경우 빈 리스트 사용
        similarities = similarities_for_rationales.get(rationale, [])
  ...
  def run(
    ...
    # ✅ 추가: rationale이 부족한 경우 처리
    if len(flattened_rationales) < num_clusters:
      _logger.warning(f'Only {len(flattened_rationales)} rationales, reducing clusters to {max(1, len(flattened_rationales))}')
      num_clusters = max(1, len(flattened_rationales))
